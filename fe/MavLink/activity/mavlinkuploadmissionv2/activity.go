...
func (a *Activity) Eval(context activity.Context) (done bool, err error)  {
	...
	// do eval
	waypointsjson2 := context.GetInput("MISSION").([]interface{})
	//fmt.Printf("[MavLinkUploadMission] %+v\n",waypointsjson2)

	jsonbody, err := json.Marshal(waypointsjson2)
	if err != nil {
		// do error check
		fmt.Println(err)
		return
	...
	waypoints := []Waypoint{}
	if err = json.Unmarshal(jsonbody, &waypoints); err != nil {
		// do error check
		fmt.Println(err)
		return
	}
	...
	err = context.SetOutput("totalmissionitems",totalmissionitems)
	if err != nil {
